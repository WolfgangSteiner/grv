...
u64 grv_strmap_hash(grv_str* str) {
    u64 hash = 5381;
...
void grv_strmap_insert(grv_strmap* map, grv_str* key, grv_str* value) {
...
void grv_strmap_insert_cstr(grv_strmap* map, char* key, char* value) {
    grv_str key_str = grv_str_new(key);
    grv_str value_str = grv_str_new(value);
    grv_strmap_insert(map, &key_str, &value_str);
}

grv_strmap_entry* grv_strmap_find(grv_strmap* map, grv_str* key) {
...
// TODO: Use a refrence instead of copying the cstring into a new grv_str
...
grv_str* grv_strmap_get(grv_strmap* map, grv_str* key) {
...
// TODO: Use a refrence instead of copying the cstring into a new grv_str
grv_str* grv_strmap_get_cstr(grv_strmap* map, char* key) {
    grv_str key_str = grv_str_new(key);
    grv_str* result = grv_strmap_get(map, &key_str);
    grv_str_free(&key_str);
    return result;
}
